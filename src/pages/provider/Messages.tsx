...
export default function ProviderMessages() {
  ...
  const { data: threads, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const customerIds = threadData.map(t => t.customer_id);
      ...
      return threadData.map(thread => ({
        ...thread,
        other_participant: profiles?.find(p => p.id === thread.customer_id) || { full_name: 'Unknown', avatar_url: null },
      }));
    },
    enabled: !!user,
    staleTime: Infinity,
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('provider-threads-updates')
      .on(
        'postgres_changes',
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {threads?.map((thread) => (
            <Link
              key={thread.id}
              to={`/provider/messages/${thread.id}`}
