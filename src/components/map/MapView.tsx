...
// Subtle LGA boundary style that blends with the map
...
// ── Custom icons ───────────────────────────────────────
function createPulsingProviderIcon(
  status: string,
      ...
      <div style="position: relative; display: flex; flex-direction: column; align-items: center; transform: translate(-50%, -100%);">
        <div style="position: relative;">
          ...
          <div style="
            ...
            border-radius: 50%;
            animation: providerPulse 2s infinite;
            z-index: 1;
          "></div>
        </div>
        <div style="
...
function UserLocationMarker() {
  ...
  useEffect(() => {
    if (!map || !permissionGranted || !lat || !lng) return;
...
function LocateButton() {
  ...
  return (
    <button
      ...
      className="absolute bottom-6 right-4 z-[1000] bg-white p-2 rounded-full shadow-lg hover:bg-gray-50 transition"
      title="My Location"
    >
      <LocateFixed className="h-5 w-5 text-primary-600" />
    </button>
  );
}

// ── Interactive LGA Boundaries layer ───────────────────
function LGABoundaries({ onLgaClick }: { onLgaClick: (lgaId: number) => void }) {
  ...
  const onEachFeature = useCallback(
    (feature: any, layer: L.Layer) => {
      ...
      layer.bindTooltip(`${lgaName}, ${stateName}`, {
        sticky: true,
        direction: 'top',
      ...
      layer.on('mouseover', () => {
        if (layer instanceof L.Path) layer.setStyle(lgaBoundaryHoverStyle);
      });
      layer.on('mouseout', () => {
        if (layer instanceof L.Path) layer.setStyle(lgaBoundaryStyle);
      });
    },
    [onLgaClick]
  );
...
export function MapView() {
  ...
  // LGA‑level focus
  const [userLgaBounds, setUserLgaBounds] = useState<L.LatLngBoundsExpression | null>(null);

  // User position (profile location preferred, else global geolocation)
  const userLat = profile?.lat ?? (permissionGranted ? globalLat : undefined);
  const userLng = profile?.lng ?? (permissionGranted ? globalLng : undefined);
  ...
  // Zoom to LGA (dropdown or map click)
  ...
  // Auto‑detect user’s LGA and set bounds + dropdowns
  useEffect(() => {
    if (!userLat || !userLng) return;

    supabase
      .rpc('find_nearest_lga', { user_lat: userLat, user_lng: userLng })
      .then(async ({ data }) => {
        if (!data || data.length === 0) return;
        const nearest = data[0];
        const lgaId: number = nearest.lga_id;
        const stateId: number = nearest.state_id;

        // Pre‑select the dropdowns
        setSelectedState(stateId.toString());
        setSelectedLga(lgaId.toString());

        // Get LGA center point
        const { data: lgaCenter } = await supabase
          .from('lga_centers')
          .select('lat, lng')
          .eq('lga_id', lgaId)
          .single();

        if (lgaCenter?.lat && lgaCenter?.lng) {
          setMapCenter([lgaCenter.lat, lgaCenter.lng]);
          setMapZoom(14);

          // Tight bounds around the LGA (~1.5 km padding)
          const pad = 0.015;
          const bounds: L.LatLngBoundsExpression = [
            [lgaCenter.lat - pad, lgaCenter.lng - pad],
            [lgaCenter.lat + pad, lgaCenter.lng + pad],
          ];
          setUserLgaBounds(bounds);
        }
      });
  }, [userLat, userLng]);

  // Effective boundary for the map (LGA focus > state fallback > Nigeria)
  const effectiveBounds = userLgaBounds || NIGERIA_BOUNDS;

  // ── Providers query ──────────────────────────────────
  const { data: allProviders, isLoading } = useQuery({
    queryKey: ['map-providers', userLat, userLng],
    queryFn: async () => {
    ...
    enabled: true,
    staleTime: 1000 * 60 * 5,      // refetch every 5 min if needed
  });

  // Filter + spread overlapping markers for a clean display
  const providersWithCoords = useMemo(() => {
    ...
    const final: MapProvider[] = [];
    groups.forEach(providers => {
      if (providers.length === 1) {
        final.push({
          ...providers[0],
          display_lat: providers[0].profile!.lat!,
          display_lng: providers[0].profile!.lng!,
        });
      } else {
        providers.forEach((p, index) => {
          const angle = (2 * Math.PI * index) / providers.length;
          const offset = 0.002;
          const lat = p.profile!.lat! + offset * Math.cos(angle);
          const lng = p.profile!.lng! + offset * Math.sin(angle);
          final.push({ ...p, display_lat: lat, display_lng: lng });
        });
      }
    });
    return final;
  ...
  return (
    <>
      ...
      <div className={cn('relative', isFullscreen ? 'fixed inset-0 z-50' : 'h-[calc(100vh-64px)]')}>
        {/* ── Top controls ────────────────────────────── */}
        <div className="absolute top-3 left-3 right-3 z-[1000] flex gap-2">
          ...
          <button
            ...
            className={cn(
              'bg-white p-2.5 rounded-lg shadow-lg border hover:bg-gray-50 transition',
              satelliteView ? 'border-primary-500 text-primary-600' : 'border-gray-200'
            )}
        ...
        <div className="flex h-full">
            ...
            >
              ...
              {providersWithCoords.map(provider => {
                ...
                const icon = isOwnProvider
                  ? createPulsingProviderIcon(
                      provider.status,
                      provider.profile?.avatar_url,
                      selectedProvider?.id === provider.id
                    )
                  : createProviderMarkerIcon(
                      provider.status,
                      provider.profile?.avatar_url,
                      selectedProvider?.id === provider.id
                    );
                    ...
                    <Popup>
                      <div className="w-60 font-body">
                        ...
                        <a
                          ...
                          className="block mt-2 text-center bg-primary-600 text-white py-1.5 rounded-md text-sm font-medium hover:bg-primary-700"
                        >
                          View Details →
                        </a>
                      </div>
            ...
            {selectedProvider && (
              <div className="absolute bottom-20 left-0 right-0 flex justify-center z-[1000] md:hidden animate-slide-up">
                <ProviderMapCard
                  ...
                  averageRating={selectedProvider.average_rating}
                  reviewCount={selectedProvider.review_count}
          ...
          {/* Provider list sidebar (desktop) */}
          {showProviderList && (
            <div className="hidden md:block w-80 border-l border-gray-200 bg-white overflow-y-auto map-provider-list">
