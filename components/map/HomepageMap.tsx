...
export default function HomepageMap({ userLocation, isExpanded = false }: HomepageMapProps) { // ACCEPT THE PROP
  ...
  return (
    <div className="w-full">
      {/* Location Status - Hide when expanded */}
      {!isExpanded && (
      ...
      {/* Map Component - FIXED: Pass isExpanded to MapSection */}
      <div className={`${isMobile ? 'rounded-xl overflow-hidden border border-gray-300 shadow-xl relative h-auto' : 'rounded-xl overflow-hidden border border-gray-300 shadow-xl relative h-[500px]'}`}>
        {/* FIXED: Pass isExpanded prop to MapSection */}
        <MapSection 
          userLocation={userLocation}
          isMobile={isMobile}
      ...
      {/* Call to Action - Hide when expanded */}
      {!isExpanded && (
